...
`no-cache` – the most misleading name on the web. This is NOT "don't cache." It means "cache it, but ask the server every time whether the cache is still valid." That is, a conditional request every time.
...
The most painful mistake – setting a long `max-age` on HTML. You deploy a new version, CSS and JS have new hashes in their filenames, but the user still sees the old HTML referencing old resources. Or even worse – resources that no longer exist. White screen, broken styles – classic.
...
*First article in the "Frontend Optimization from First Click" series.*
