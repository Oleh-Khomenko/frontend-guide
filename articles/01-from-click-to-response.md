# Оптимізація фронтенду від першого кліку – Частина 1.

Коли я намагався розібратися в оптимізації фронтенду, мене здивувало одне: інформації багато, але вона розкидана і часто поверхнева. Одна стаття пояснює DNS, інша – кешування, третя – рендеринг, але рідко яка заглиблюється далі загальних схем – наприклад, як саме працює код браузера, який будує render tree чи робить compositing. І жодна не зв'язує ці теми в єдиний ланцюжок: від кліку до пікселя на екрані. В результаті ти знаєш окремі факти, але не бачиш повної картини. Ця серія – моя спроба зібрати все, що я знаю, структурувати і вибудувати в логічну послідовність – для себе і для всіх, кому це стане в пригоді.

# Від кліку до відповіді: що відбувається до першого байта
...
Тут ми розберемо, що відбувається між кліком і першим байтом відповіді від сервера – покроково, з конкретними цифрами, граблями і способами їх обійти.

---

## 1. Клік – і годинник пішов

Все починається з дії: клік на посилання, введення URL, кнопка "Назад" або програмний [`window.location`](https://developer.mozilla.org/en-US/docs/Web/API/Window/location). Браузер фіксує цю мить як `startTime` (завжди `0`) у [Navigation Timing API](https://developer.mozilla.org/en-US/docs/Web/API/PerformanceNavigationTiming) – і від неї рахуються всі подальші метрики.
...
**Memory cache** – оперативна пам'ять вкладки. Блискавичний, але живе тільки поки відкрита вкладка. Якщо один і той самий скрипт або зображення використовується на кількох сторінках сайту – при переході на наступну сторінку ресурс вже в пам'яті, і браузер не робить повторний запит.
...
**[Service Worker](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) cache** – ваш власний, програмований шар. SW перехоплює запити і вирішує: віддати з кешу, піти в мережу, або скомбінувати (віддати кеш зараз, а у фоні оновити). Потужна штука, але і складна – баги в кешуванні через Service Worker найважчі для дебагу, бо користувач бачить застарілий контент і не розуміє, чому. Але є важливий нюанс: SW реєструється при першому візиті, а контролювати сторінку починає лише з наступної навігації (якщо SW не викличе [`clients.claim()`](https://developer.mozilla.org/en-US/docs/Web/API/Clients/claim) – тоді він перехопить контроль одразу). Тобто при першому заході на сайт цей шар кешу зазвичай не працює.

Окремо варто згадати **[bfcache](https://web.dev/articles/bfcache)** (back/forward cache). Це не кеш ресурсів – це знімок усієї сторінки в пам'яті: DOM, JavaScript heap, стан layout. Коли користувач натискає "Назад" або "Вперед", браузер відновлює сторінку з bfcache миттєво, без жодних мережевих запитів і без повторного парсингу. Щоб не ламати bfcache, уникайте `unload` listeners – це основна причина, через яку браузер відмовляється зберігати сторінку в bfcache. Раніше `Cache-Control: no-store` також блокував bfcache, але Chrome 116+ ігнорує цю директиву для bfcache, зберігаючи сторінку в пам'яті незалежно від неї.
...
Коли кеш прострочений (вийшов `max-age`) або стоїть `no-cache`, браузер не завантажує ресурс заново одразу. Він робить conditional request – надсилає запит з одним із двох заголовків:

- [`If-None-Match`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/If-None-Match) – містить `ETag` (унікальний ідентифікатор версії ресурсу – може бути хешем контенту, revision number або іншим значенням на розсуд сервера), який сервер віддав минулого разу. Сервер порівнює зі своїм поточним ETag – якщо збігається, ресурс не змінився.
- [`If-Modified-Since`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/If-Modified-Since) – містить дату `Last-Modified` з попередньої відповіді. Сервер перевіряє, чи змінювався ресурс після цієї дати.

Якщо ресурс не змінився – сервер відповідає **304 Not Modified** з порожнім тілом. Замість 200 КБ JavaScript по мережі летить кілька сотень байт заголовків. Суттєва економія.
...
Головна порада – менше доменів. Кожен third-party скрипт зі свого домену – окремий DNS-запит. Google Analytics, шрифти з Google Fonts, рекламний SDK – три домени, три резолви. Вони йдуть паралельно, але кожен може затримати свій ланцюжок залежних ресурсів на 30-100 мс.

<a id="dns-prefetch"></a>Для доменів, яких не уникнути, є [`dns-prefetch`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel/dns-prefetch):
...
**[HTTP/2](https://developer.mozilla.org/en-US/docs/Glossary/HTTP_2) зробив це непотрібним.** Одне з'єднання – і через нього паралельно летять усі запити (мультиплексування). Один handshake замість шести. Якщо ви досі на HTTP/1.1 – перехід на HTTP/2 це, мабуть, найбільший виграш за найменші зусилля. Більшість CDN підтримують його з коробки.

### [TCP](https://developer.mozilla.org/en-US/docs/Glossary/TCP): потрійне рукостискання
...
Один round trip. При [RTT](https://developer.mozilla.org/en-US/docs/Glossary/Round_Trip_Time) 100 мс до сервера – 100 мс тільки на рукостискання. І це ще без шифрування.

### [TLS](https://developer.mozilla.org/en-US/docs/Glossary/TLS): ще рукостискання поверх
...
Ця різниця напряму впливає на [TTFB](https://developer.mozilla.org/en-US/docs/Glossary/Time_to_first_byte) (Time to First Byte) – браузер не може надіслати HTTP-запит, поки не завершить handshake. На мобільному з'єднанні з RTT 150 мс перехід з TLS 1.2 на 1.3 скорочує TTFB на 150 мс – просто за рахунок одного зекономленого round trip.

### [HTTP/3](https://developer.mozilla.org/en-US/docs/Glossary/HTTP_3) і [QUIC](https://developer.mozilla.org/en-US/docs/Glossary/QUIC)
...
Це повний handshake (DNS + TCP + TLS) заздалегідь. Дорожче за [`dns-prefetch`](#dns-prefetch), тому використовуйте точково – 2-3 найважливіших origin, не більше. Інакше ви витратите bandwidth на з'єднання, які можуть і не знадобитися.
...
## 6. [HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP)-запит і відповідь
...
Окрема тема – cookies. Вони прикріплюються до **кожного** запиту до домену. Не тільки до HTML – до кожного зображення, CSS-файлу, JS-чанку. Якщо у вас 4 КБ cookies і на сторінці 50 ресурсів з вашого домену – на HTTP/1.1 це 200 КБ додаткового трафіку тільки на заголовки запитів. HTTP/2 стискає повторювані заголовки через [HPACK](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Connection_management_in_HTTP_2#header_compression), тому реальний overhead значно менший, але початковий cookie все одно летить повністю, і великі cookies збільшують час першого запиту.
...
*Перша стаття з серії "Оптимізація фронтенду від першого кліку".*
